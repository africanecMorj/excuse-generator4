...
app.use(express.static(path.join(__dirname, '..', 'components', 'Admin', 'Admin.jsx')));
...
app.post('/excuse-add', (req, res) => {
    ...
    let filteredExcuse = excuse.split(' ').filter(word => word != /[!@#$%^&*()_+-=~`'"]/)
...
try{
    ...
    let authors = [];
    excuses.forEach((i) => {
        i.author != undefinded ? excuse.push(i.author) : {}; 
    });
    res.send(authors);
} catch(err) {
...
app.get(`/like/:id`, (req, res) => {
    ...
    // Excuse.findOneAndUpdate({_id:id}, )
})
